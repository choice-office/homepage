...
## 1. 데이터 모델 (`lib/blog.ts` `BlogPost` · Supabase 매핑)
...
## 구현 상태
- ✅ 공개 렌더 + JSON-LD + **Supabase 읽기**: 구현됨. 데이터는 `blog_posts`(스키마 `docs/BLOG.md`)에서 `lib/blog.ts`로 읽고, 옵션 필드는 채우면 렌더·스키마에 반영, 비우면 생략/기본값.
