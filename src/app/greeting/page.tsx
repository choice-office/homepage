					...
					<div data-stagger="split" className={cn("grid-2 greeting-split", "items-stretch")}>
						<div className="flex flex-col justify-center">
							...
							<div className="mt-7 flex flex-col gap-[18px] text-[16.5px] text-[color:var(--text-body)] leading-[1.9]">
								{/* 사무소명은 갈라지지 않게 — 좁은 폭에서 '초이스'만 앞줄에 남던 문제 */}
								<p>안녕하십니까. '실력에 책임감을 더한' 초이스&nbsp;행정사사무소입니다.</p>
								<p>
									살다 보면 익숙하지 않은 행정 절차 앞에서 무엇부터 시작해야 할지, 어떤 정보가
									정확한지 판단하기 어려운 막막한 순간이 있습니다.
									{/* 모바일에서 "초이스 행정사사무소는 그런 상황에서"가 새 줄에서 시작하게(PC 는 흐름 유지) */}
									<br className="sm:hidden" /> 초이스 행정사사무소는 그런 상황에서 의뢰인의 입장에서
									함께 길을 찾아드리는 역할을 하고 있습니다.
								</p>
								{/* &nbsp; 는 줄 끝에 홀로 남으면 어색한 어절을 뒤 어절과 묶는 것(관형사 '각' 등).
								    JSX 주석을 텍스트 사이에 두면 인접 공백이 사라지므로 문장 밖에 적는다. */}
								<p>
									2019년 사무소 개소 이후 외국인 체류 및 비자, 출입국 관련 행정 업무를 중심으로
									다양한 사례를 직접 다뤄오며 실무 경험을 쌓아왔습니다.
									{/* 모바일에서 "이러한 경험을 바탕으로"가 새 줄에서 시작하게(PC 는 흐름 유지) */}
									<br className="sm:hidden" /> 이러한 경험을 바탕으로 의뢰인분들의 각&nbsp;상황에
									맞는 현실적인 방향을 안내드리고 있습니다.
								</p>
								{/* 맺음말은 한 줄로 내린다 */}
								<p>
				...
				<div className="wrap">
					<div className="tm-grid">
						...
						<div className="tm-body">
							...
							<p className="tm-lead">
								초이스&nbsp;행정사사무소
								<span className="tm-en max-sm:hidden">CHOI’S Administrative Attorney Office</span>는
								2019년 개업과 동시에 상표를 출원하고,
								<br />
							...
							<details className="tm-law">
								{/* summary 는 flex('+' 마커 + 본문) — 본문을 span 하나로 감싸 한 항목으로 유지한다.
								    쪼개면 마커 옆에 두 칼럼처럼 늘어선다. 좁은 폭에서만 조문 앞에서 줄을 바꾼다. */}
								<summary>
									<span>
										「상표법」 제6장 상표권자의 보호 —
										<br className="sm:hidden" /> 제107조·제108조·제109조·제111조~제113조
									</span>
								</summary>
								<ul>
									{TRADEMARK_LAW.map((l) => (
